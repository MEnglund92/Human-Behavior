"""Phase 2 generator: per-chapter deep extraction for The Righteous Mind
(Jonathan Haidt).

Adds one chapter entry (rm00..rm12) per book section (Introduction, Ch1..Ch12),
each with 5 assets (one per asset type), while keeping the existing topical
entries (rm_intro, rm_ch01..rm_ch12, rm_conclusion) untouched.

Run from the repo root:
    python extract/tools/gen_righteous_p2.py
"""
...
OUT = os.path.join(HERE, "..", "generated_assets", "righteous_mind_assets.json")

sys.path.insert(0, HERE)
from righteous_p2_data_a import CHAPTERS as DATA_A
from righteous_p2_data_b import CHAPTERS as DATA_B
from righteous_p2_data_c import CHAPTERS as DATA_C
