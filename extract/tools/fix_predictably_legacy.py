"""Fix predictably_irrational_assets.json legacy assets (ch_intro, ch01..ch15):
add player_mission, difficulty_level, visual_frame_description, and expand
...
PATH = os.path.join(HERE, "..", "generated_assets", "predictably_irrational_assets.json")

sys.path.insert(0, HERE)
from predictably_p2_data_a import CHAPTERS as A
from predictably_p2_data_b import CHAPTERS as B
from predictably_p2_data_c import CHAPTERS as C
...
def main():
    ...
    for ch in data["chapters"]:
        ...
        if cid == "ch_intro":
            num = 0
        elif cid.startswith("ch") and cid[2:].isdigit():
            num = int(cid[2:])
        else:
            continue
        pool = pool_for(num)
