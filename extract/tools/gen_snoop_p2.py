#!/usr/bin/env python3
"""Phase 4 generator: per-chapter deep extraction for Snoop: What Your Stuff
Says About You (Gosling, 2008).

One authoring agent per chapter produced extract/tools/snoop_p2_chNN.py
(each defines a single CHAPTER dict). This generator loads all of them and
writes the dict-style library to generated_assets/snoop_assets.json.

Run from the repo root:
    python extract/tools/gen_snoop_p2.py
"""
...
from snoop_p2_map import CHAPTERS as MAP

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "..", "generated_assets", "snoop_assets.json")
...
def load_chapters():
    chapters = []
    for path in sorted(glob.glob(os.path.join(HERE, "snoop_p2_ch*.py"))):
...
def build_chapter_entry(ch, num):
    ...
    return {
        "id": "snoop%02d" % num,
        "title": ("Prologue: The Arrival of the Mystery Box" if num == 1 else "Chapter %d: %s" % (ch["num"], ch["title"])),
        "pages": ch["pages"],
        "assets": assets,
    }
...
def main():
    ...
    data = {
        "source": "Snoop: What Your Stuff Says About You (Gosling, 2008)",
