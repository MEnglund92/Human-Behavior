#!/usr/bin/env python3
"""Phase 4 generator: per-chapter deep extraction for Self-Presentation:
Impression Management and Interpersonal Behavior (Leary, 1996).

One authoring agent per chapter produced extract/tools/selfpres_p2_chNN.py
(each defines a single CHAPTER dict). This generator loads all of them and
writes the dict-style library to generated_assets/self_presentation_assets.json.

Run from the repo root:
    python extract/tools/gen_selfpres_p2.py
"""
...
from selfpres_p2_map import CHAPTERS as MAP

HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "..", "generated_assets", "self_presentation_assets.json")
...
def load_chapters():
    chapters = []
    for path in sorted(glob.glob(os.path.join(HERE, "selfpres_p2_ch*.py"))):
...
def main():
    ...
    data = {
        "source": "Self-Presentation: Impression Management and Interpersonal Behavior (Leary, 1996)",
