"""Phase 2 generator: per-chapter deep extraction for The Social Animal
(Elliot Aronson).

Adds one chapter entry (sa01..sa09) per book chapter (Ch1..Ch9), each with 5
assets (one per asset type), while keeping the existing topical entries
(sa_ch01a..sa_ch09b) untouched.

Run from the repo root:
    python extract/tools/gen_socialanimal_p2.py
"""
...
OUT = os.path.join(HERE, "..", "generated_assets", "social_animal_assets.json")

sys.path.insert(0, HERE)
from socialanimal_p2_data_a import CHAPTERS as DATA_A
from socialanimal_p2_data_b import CHAPTERS as DATA_B
