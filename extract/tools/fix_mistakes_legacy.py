"""Fix mistakes_assets.json legacy assets (ch00a..ch09): add player_mission,
difficulty_level, visual_frame_description, and expand key_concepts 8-12 from
Phase-2 chapter data."""
...
PATH = os.path.join(HERE, "..", "generated_assets", "mistakes_assets.json")

sys.path.insert(0, HERE)
from mistakes_p2_data_a import CHAPTERS as A
from mistakes_p2_data_b import CHAPTERS as B
...
def main():
    ...
    for ch in data["chapters"]:
        cid = ch["id"]
        if not (cid.startswith("ch") and len(cid) >= 3 and cid[2:4].isdigit()):
            continue
        num = int(cid[2:4])
        pool = pool_for(num)
        for a in ch["assets"]:
