"""Phase 2 generator: per-chapter deep extraction for Influence (Robert Cialdini).

Adds one chapter entry (in00..in09) per book section (Introduction, Ch1..Ch9),
each with 5 assets (one per asset type), while keeping the existing topical
entries (ch01a..ch09b) untouched.

Run from the repo root:
    python extract/tools/gen_influence_p2.py
"""
...
OUT = os.path.join(HERE, "..", "generated_assets", "influence_assets.json")

sys.path.insert(0, HERE)
from influence_p2_data_a import CHAPTERS as DATA_A
from influence_p2_data_b import CHAPTERS as DATA_B
