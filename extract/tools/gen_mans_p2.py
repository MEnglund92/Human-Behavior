"""Phase 2 generator: per-section deep extraction for Man's Search for Meaning
(Viktor E. Frankl).

Adds one chapter entry (mf01..mf03) per book section (Part 1, Part 2,
Postscript), each with 5 assets (one per asset type), while keeping the
existing topical entries (part1, part2, postscript) untouched.

Run from the repo root:
    python extract/tools/gen_mans_p2.py
"""
...
OUT = os.path.join(HERE, "..", "generated_assets", "mans_search_meaning_assets.json")

sys.path.insert(0, HERE)
from mans_p2_data_a import CHAPTERS as DATA_A
