...
- definitive_body_language_assets.json — 200 assets, ALL VALID (0 warnings)
- what_every_body_assets.json — 95 assets, ALL VALID (0 warnings)
- attached_workbook_assets.json — 90 assets, ALL VALID (0 warnings)
...
- behave_assets.json — 126 assets, ALL VALID (0 warnings; Phase 2: bh00-bh18)
- influence_assets.json — 101 assets, ALL VALID (0 warnings; Phase 2: in00-in09)
- laws_human_nature_assets.json — 118 assets, ALL VALID (0 warnings; Phase 2: lh00-lh18)
- mans_search_meaning_assets.json — 25 assets, ALL VALID (0 warnings; Phase 2: mf01-mf03)
- mistakes_assets.json — 86 assets, ALL VALID (0 warnings; Phase 2: mk00-mk09)
- moral_animal_assets.json — 125 assets, ALL VALID (0 warnings; Phase 2: ma00-ma18)
- predictably_irrational_assets.json — 102 assets, ALL VALID (0 warnings; Phase 2: pi00-pi15)
- social_animal_assets.json — 92 assets, ALL VALID (0 warnings; Phase 2: sa01-sa09)
- righteous_mind_assets.json — 104 assets, ALL VALID (0 warnings; Phase 2: rm00-rm12)
- social_intelligence_assets.json — 137 assets, ALL VALID (0 warnings; Phase 2: si00-si22)
...
- **Phase 2 complete — no books remain.** Both Phase-2 targets (Bowden, Glass,
  Reiman) are done.
- **What Every BODY Is Saying (Navarro) — DONE 2026-07-31.** 50 → 95 assets.
...
- **Behave (Sapolsky) — DONE 2026-07-31.** 31 → 126 assets. Book re-read from
...
- **Influence (Cialdini) — DONE 2026-07-31.** 51 → 101 assets. Book re-read
...
- **Laws of Human Nature (Greene) — DONE 2026-07-31.** 23 → 118 assets. Book
...
- **Man's Search for Meaning (Frankl) — DONE 2026-08-01.** 10 → 25 assets.
...
- **Mistakes Were Made (Tavris & Aronson) — DONE 2026-08-01.** 36 → 86 assets.
...
- **The Moral Animal (Wright) — DONE 2026-08-01.** 30 → 125 assets. Book
...
- **Predictably Irrational (Ariely) — DONE 2026-08-01.** 22 → 102 assets.
...
- **The Social Animal (Aronson) — DONE 2026-08-01.** 47 → 92 assets. Book
...
- **The Righteous Mind (Haidt) — DONE 2026-08-01.** 39 → 104 assets. Scanned
...
- **Social Intelligence (Goleman) — DONE 2026-08-01.** 22 → 137 assets.
...
- **Attached workbook (Levine & Heller) — DONE 2026-07-31.** 45 → 90 assets.
...
- `extract\tools\gen_reiman_p2.py` + `reiman_p2_data_{a..d}.py` — Phase 2
  Reiman generator and its per-chapter content data
- `extract\tools\gen_definitive_p2.py` + `definitive_p2_data_{a..e}.py` —
  Phase 2 Definitive generator and its per-chapter content data
- `extract\tools\gen_whatbody_p2.py` + `whatbody_p2_data_{a..c}.py` — Phase 2
  What Every BODY generator and its per-chapter content data
- `extract\tools\gen_attached_p2.py` + `attached_p2_data_{a..b}.py` — Phase 2
  Attached workbook generator and its per-chapter content data
- `extract\tools\gen_behave_p2.py` + `behave_p2_data_{a..c}.py` — Phase 2
  Behave generator and its per-chapter content data
- `extract\tools\gen_influence_p2.py` + `influence_p2_data_{a..b}.py` — Phase 2
  Influence generator and its per-chapter content data
- `extract\tools\gen_laws_p2.py` + `laws_p2_data_{a..c}.py` — Phase 2 Laws of
  Human Nature generator and its per-chapter content data
- `extract\tools\gen_mans_p2.py` + `mans_p2_data_a.py` — Phase 2 Man's Search
  for Meaning generator and its per-section content data
- `extract\tools\gen_mistakes_p2.py` + `mistakes_p2_data_{a..b}.py` — Phase 2
  Mistakes Were Made generator and its per-chapter content data
- `extract\tools\gen_moral_p2.py` + `moral_p2_data_{a..c}.py` — Phase 2 Moral
...
- `extract\tools\gen_socialanimal_p2.py` + `socialanimal_p2_data_{a..b}.py` —
  Phase 2 Social Animal generator and its per-chapter content data
- `extract\tools\gen_righteous_p2.py` + `righteous_p2_data_{a..c}.py` — Phase 2
  Righteous Mind generator and its per-chapter content data (OCR source)
- `extract\tools\gen_socialint_p2.py` + `socialint_p2_data_{a..e}.py` — Phase 2
...
- `extract\tools\fix_{mans,mistakes,moral,predictably,socialanimal,righteous,socialint}_legacy.py`
  — M5 legacy backfill scripts (player_mission <- question, difficulty per
...
- `extract\tools\run_app_stub.js` — M7 bugfix: executes the inline app script
